  ...
  const handleSubmitOTP = (values: z.infer<typeof otpSchema>) => {
    ...
    setTimeout(() => {
      ...
      toast({
        ...
        description: "Votre réservation a été confirmée. Vous recevrez votre billet rapidement.",
      });
      onComplete();
    }, 2000);
  };
    ...
    if (paymentData.method === "card") {
      return (
        <div className="space-y-4 mt-6">
          <p className="text-off-white mb-4">Veuillez vérifier vos informations de paiement:</p>
          
          <div className="p-4 bg-black/30 rounded-md">
            <div className="grid grid-cols-2 gap-2 text-sm">
              <span className="text-off-white/70">Type de carte:</span>
              <span className="text-off-white font-medium">Visa/Mastercard</span>
              ...
              <span className="text-off-white font-medium">{paymentData.cardExpiry}</span>
            </div>
          </div>
    ...
    switch (paymentData.provider) {
      ...
      default:
        ...
        return (
          <div className="space-y-4 mt-6">
            <p className="text-off-white mb-4">
              Entrez le code OTP reçu par SMS pour confirmer le paiement avec {paymentData.provider}
            </p>
            
            <Form {...form}>
              <form onSubmit={form.handleSubmit(handleSubmitOTP)} className="space-y-6">
                <FormField
  ...
  return (
    <div>
      ...
      <div className="p-4 bg-black/30 rounded-md mb-6">
        <h3 className="font-medium text-gold mb-3">Récapitulatif de la commande</h3>
        <div className="grid grid-cols-2 gap-2 text-sm">
          ...
          <span className="text-off-white capitalize">
            {paymentData.method === "mobile_money" 
              ? `${paymentData.provider?.charAt(0).toUpperCase()}${paymentData.provider?.slice(1)}`
              : "Carte bancaire"}
          </span>
          ...
          <span className="text-gold font-bold pt-2 border-t border-titanium/20 mt-2">
