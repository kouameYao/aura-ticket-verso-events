...
const EventCard = ({ event }: { event: EventProps }) => {
  return (
    <Card className={`overflow-hidden transition-all duration-300 hover:shadow-lg hover:shadow-gold/20 ${
      ...
      <div className="p-4">
        <h3 className="font-playfair text-lg font-bold mb-2 text-off-white line-clamp-1">{event.title}</h3>
        ...
        <div className="mt-4 pt-4 border-t border-titanium/20 flex items-center justify-between">
          ...
          <button className="px-3 py-1 bg-bordeaux hover:bg-bordeaux/80 text-off-white text-sm rounded transition-colors">
            Réserver
          </button>
        </div>
      </div>
